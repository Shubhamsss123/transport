var regPrice = 17700;
var fees_others = 17700;
var fees_stdwp = 8850;
var regType = "ON SPOT REGISTRATION";
...
$('#regsubmitbtn').on('click', function(e){
    ...
    if($('#uname').val() && $('#uemail').val() && $('#uphone').val() && $('#uorg').val() && $('#addr').val() && $('#pin').val() && $('#otsfeeinp').val()){   //&& formvalidity == "valid"){
        var currency = "INR";
	...
	var ebtoken = regType + Math.floor(Math.random()*(999999-100000+1)+100000);
	    ...
	    'addr' : $('#addr').val(),
            'pin' : $('#pin').val(),
            'totalcharges' : $('#otsfeeinp').val(), //regPrice replace with otsfeeinp for auto fee input,
            'currency' : currency,
            'paperid' : paperid,
	    'studentid' : studentid,
	    'token' : ebtoken
